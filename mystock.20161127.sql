...
CREATE TABLE `relation_stocks` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=MyISAM AUTO_INCREMENT=51 DEFAULT CHARSET=utf8 COMMENT='相关股票';
...
CREATE TABLE `stock_finance_data` (
  ...
  `created_time` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`)
) ENGINE=MyISAM AUTO_INCREMENT=1000 DEFAULT CHARSET=utf8 COMMENT='财务数据（网易）';
...
CREATE TABLE `stock_holder` (
  ...
  `holder_parent_type` varchar(100) DEFAULT NULL COMMENT '股东机构类型',
  `report_date` date DEFAULT NULL COMMENT '报告日期',
  `created_time` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`)
) ENGINE=MyISAM AUTO_INCREMENT=974664 DEFAULT CHARSET=utf8 COMMENT='流通股东表（新浪）';
...
CREATE TABLE `stock_holder_sum` (
  ...
  `created_time` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`)
) ENGINE=MyISAM AUTO_INCREMENT=5554 DEFAULT CHARSET=utf8 COMMENT='机构流通股东汇总表（新浪，统计最近一期的持股情况）';
...
CREATE TABLE `stock_trade_data` (
  ...
  `created_time` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`)
) ENGINE=MyISAM AUTO_INCREMENT=62834 DEFAULT CHARSET=utf8 COMMENT='行情数据（网易）';
